...
* **`IllegalArgumentException`**: โยนเมื่อส่ง `capacity <= 0` หรือเมื่อสั่ง `push(null)`
...
`peek()` คืนค่า element ตรงๆ ไม่ได้ copy ให้ใหม่ ถ้า `E` เป็นชนิดที่แก้ไขได้ (เช่น `int[]`) แล้ว client เอาค่าที่ได้จาก peek() ไปแก้ จะกระทบข้อมูลจริงในสแตกทันที เพราะเป็น object ตัวเดียวกัน
